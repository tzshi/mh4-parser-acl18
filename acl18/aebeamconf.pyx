...
cdef class AEBeamConf:
    ...
    cpdef void init_conf(self, int pos, int rootfirst=0, int direction=L2R):
        cdef int i
        cdef int length = self.length
        self.step[pos] = 0
    ...
    cpdef np.ndarray[ndim=1, dtype=np.npy_intp] gold_transitions(self, int pos, int rootfirst=0, int direction=L2R):
        ...
        self.init_conf(pos, rootfirst, direction)
