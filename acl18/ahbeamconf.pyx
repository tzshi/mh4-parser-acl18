...
cdef class AHBeamConf:
    ...
    cpdef np.ndarray[ndim=1, dtype=np.npy_intp] valid_transitions(self, int pos):
        ...
        if self.buffer_head[pos] > 0 and self.stack_head[pos] > 0 and \
                self.stack[offset+self.stack_head[pos]-1] != 0:
            ret[1] = 1
    ...
    cpdef np.ndarray[ndim=1, dtype=np.npy_intp] transition_costs(self, int pos):
        ...
        if valid[0]:
            #shift
            cost = 0
            if self.gold_heads[b0] >= 0 and self.location[offset+self.gold_heads[b0]] == STACK_LOC and \
                    self.gold_heads[b0] != s0:
                cost += 1
            for i in range(self.stack_head[pos]):
                parent = self.gold_heads[self.stack[offset+i]]
                if parent >= 0 and parent == b0:
                    cost += 1
            ret[0] = cost
        if valid[1]:
            #leftarc
        ...
        if valid[2]:
            ...
            for i in range(self.buffer_head[pos]):
                ...
                if parent >= 0 and parent == s0:
                    cost += 1
            parent = self.gold_heads[s0]
            if parent >= 0 and self.location[offset+parent] == BUFFER_LOC:
                cost += 1
            ret[2] = cost
        return ret
