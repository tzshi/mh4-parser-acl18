...
ONEEC_MLP_ACTIVATION=tanh
ONEEC_MLP_DIMS=192
ONEEC_MLP_LAYERS=1
ONEEC_MLP_DROPOUT=0.3
...
LOG_FOLDER=/home/ts678/nonproj/models/cdparser/oneec/
...
MKL_NUM_THREADS=1 python -m acl18.cdparser \
    ...
    - create-parser --batch-size $BATCH_SIZE --word-smooth $WORD_SMOOTH \
        ...
        --mh4t-num $MH4T_NUM \
        --mh4t-mode $MH4T_MODE \
        --oneec-mlp-activation $ONEEC_MLP_ACTIVATION --oneec-mlp-dims $ONEEC_MLP_DIMS \
        --oneec-mlp-layers $ONEEC_MLP_LAYERS --oneec-mlp-dropout $ONEEC_MLP_DROPOUT \
        --oneec-num $ONEEC_NUM \
        ...
        --oneec-weight $ONEEC_WEIGHT \
        --as-weight $AS_WEIGHT \
        --label-weight $LABEL_WEIGHT --utagger_weight $UTAGGER_WEIGHT --xtagger-weight $XTAGGER_WEIGHT \
    - init-model \
    - train $TRAIN_FILE --dev $DEV_FILE \
        --utag True --xtag True --ast False --ahdp False --aedp False --mst False --npmst False --mh4 False --mh4t False --oneec True --label True \
        --dev-portion 1.0 \
